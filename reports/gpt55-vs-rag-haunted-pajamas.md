...
| Socartes full-novel automatic index | 10/10 | 29/30 | 39/40 | 2/2 | chunk ID |

The main RAG result after removing answer-key-like heuristics is 39/40 answerable retrieval and 2/2 correct refusal on the full-novel automatic index. The single answerable miss is Q27, where the question uses `fight` but the evidence uses Jenkins' word `scrimmage`; this is a semantic recall miss rather than a corpus coverage failure.
...
| Hybrid RRF + rerank without support gate | 10/10 | 29/30 | 39/40 | 0/2 |
| Full setting: hybrid RRF + rerank plus support gate | 10/10 | 29/30 | 39/40 | 2/2 |

The ablation separates the two failure modes. The legacy top-1 baseline misses Q2, Q5, and Q10 because the answer phrase is in a lower-ranked or adjacent chunk. Hybrid RRF recall and reranking recover those answer passages and most of the harder Q13-Q42 extension set, but they also widen the evidence window enough for both unsupported controls to become false hits. The direct-support gate fixes this by refusing Q11 and Q12 when the retrieved evidence does not directly support the requested target.
...
The later Q23-Q32 extension exposed the next limit of pure lexical recall. Q23 first appeared at lexical rank 287, Q27 at rank 542, and Q29 at rank 67. Q24 appeared at rank 29 but required the reranker to prefer the exact "Miss Billings" correction passage over generic Frances/Lightnut context. Hybrid RRF recall brings vector candidates into the candidate pool, and support-aware reranking selects most passages that directly answer the question. After removing hardcoded answer synonyms such as `scrimmage`, Q27 remains incorrect because the current retrieval stack does not bridge the paraphrase from `fight` in the question to `scrimmage` in the passage. The added Q33-Q42 diagnostic stays correct under the same pipeline.
...
The main claim should stay scoped to this implementation and corpus: Socartes can ground 39 of 40 answerable diagnostic questions in a local full-novel index and refuse the two unsupported controls after answer-key-like heuristic synonyms are removed. Broader robustness still requires follow-up work such as larger multi-corpus tests, learned cross-encoder reranking, and stronger semantic support checking.
